...
const App: React.FC = () => { 
  ...
  const handleLogin = () => {
    if (userField === "instrutor" && passField === "gtm2026") {
  ...
  const protocols = [
    { level: 'FÁCIL', q: 'Qual a prioridade principal da GTM em acompanhamentos?', a: 'A GTM tem prioridade em acompanhamentos à motocicletas.' },
    ...
    { level: 'FÁCIL', q: 'Quantas motocicletas podem compor, no máximo, uma unidade GTM?', a: 'Até três motocicletas (P1, P2 e P3).' },
    ...
    { level: 'DIFÍCIL', q: 'Concurso para outro grupamento?', a: 'Deve pedir baixa da GTM antes, sujeito a exoneração.' },
  ...
  const handleFinish = async () => {
    ...
    try {
      await addDoc(collection(db, "evaluations"), {
        ...
        finalScore,
        passingGrade,
        review: { summary: "Avaliação registrada via Sistema GTM." }
      });
  ...
  if (!isLogged) {
    return (
      <div className="h-screen w-full flex items-center justify-center bg-black font-sans">
        <div className="bg-zinc-900/50 border border-purple-500/20 p-10 rounded-[2.5rem] w-full max-w-sm text-center backdrop-blur-md shadow-2xl">
          <img src="https://i.imgur.com/su8eRVi.png" alt="GTM" className="w-20 h-20 mx-auto mb-6 object-contain" />
          <h2 className="text-purple-500 font-bold tracking-widest text-xl mb-8 uppercase">Acesso Restrito</h2>
          <div className="space-y-4">
            <input placeholder="USUÁRIO" className="w-full bg-black border border-white/5 rounded-xl p-3.5 text-[10px] font-black text-white outline-none focus:border-purple-500/50" onChange={e => setUserField(e.target.value)} />
            <input type="password" placeholder="SENHA" className="w-full bg-black border border-white/5 rounded-xl p-3.5 text-[10px] font-black text-white outline-none focus:border-purple-500/50" onChange={e => setPassField(e.target.value)} />
            <button onClick={handleLogin} className="w-full py-4 bg-purple-500 text-black rounded-xl font-black text-[10px] tracking-[0.2em] hover:bg-purple-400 mt-4">AUTENTICAR</button>
          </div>
        </div>
      </div>
    );
    ...
    <div className="h-screen w-full flex flex-col bg-black text-slate-300 overflow-hidden font-sans select-none">
      ...
      {showSettings && (
        <div className="fixed inset-0 z-[150] flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="bg-zinc-900 border border-white/10 p-8 rounded-[2rem] w-full max-w-xs space-y-6 shadow-2xl">
            <h3 className="text-[10px] font-black text-purple-500 uppercase tracking-widest">Controles do Sistema</h3>
      ...
      {showResultModal && selectedResult && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/95 backdrop-blur-2xl animate-in fade-in duration-300">
          <div className="bg-zinc-950 border border-purple-500/30 w-full max-w-4xl rounded-[2.5rem] overflow-hidden flex flex-col shadow-[0_0_100px_rgba(245,158,11,0.05)] max-h-[95vh]">
            ...
            <div className="p-10 flex-1 overflow-y-auto custom-scrollbar space-y-10">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                <div className="space-y-6">
                  <div>
                    <span className="text-[10px] font-black text-purple-500 uppercase tracking-widest block mb-1">Candidato Avaliado</span>
                  ...
                  <div className="flex items-center gap-6 bg-zinc-900/40 p-8 rounded-[2rem] border border-white/5">
                    <div className="text-center pr-8 border-r border-white/10">
                      ...
                      <div className="text-5xl font-black text-purple-500">{selectedResult.finalScore.toFixed(1)}</div>
                    </div>
                ...
                <div className="bg-zinc-900/20 p-8 rounded-[2rem] border border-white/5 space-y-4">
                  <h4 className="text-[10px] font-black text-purple-500 uppercase tracking-widest flex items-center gap-2"><Trophy className="w-4 h-4"/> Parecer Tático</h4>
              ...
              <div className="space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  <div className="bg-zinc-900/40 p-6 rounded-2xl border border-white/5">
                    <span className="text-[9px] font-black text-purple-500 uppercase tracking-widest block mb-4 border-b border-white/5 pb-2">Rampas</span>
                    {selectedResult.state.ramps.map((r, i) => (
                      <div key={i} className="flex justify-between text-[11px] font-bold uppercase mb-1 last:mb-0">
                        ...
                        <span className="text-purple-500">{r.score.toFixed(1)}</span>
                      </div>
                    ))}
                  </div>
                  <div className="bg-zinc-900/40 p-6 rounded-2xl border border-white/5">
                    <span className="text-[9px] font-black text-purple-500 uppercase tracking-widest block mb-4 border-b border-white/5 pb-2">Circuitos</span>
                    <div className="flex justify-between text-[11px] font-bold uppercase mb-2">
                      <span className="text-slate-500">TÚNEL</span>
                      <span className="text-purple-500">{selectedResult.state.tunnel.score.toFixed(1)}</span>
                    </div>
                    <div className="flex justify-between text-[11px] font-bold uppercase">
                      <span className="text-slate-500">CHINA</span>
                      <span className="text-purple-500">{selectedResult.state.modulation.score.toFixed(1)}</span>
                    </div>
                  </div>
                  <div className="bg-zinc-900/40 p-6 rounded-2xl border border-white/5">
                    <span className="text-[9px] font-black text-purple-500 uppercase tracking-widest block mb-4 border-b border-white/5 pb-2">Acomp.</span>
                    {selectedResult.state.tracking.map((t, i) => (
                      <div key={i} className="flex justify-between text-[11px] font-bold uppercase mb-1 last:mb-0">
                        <span className="text-slate-500">{t.location || 'SETOR'}</span>
                        <span className="text-purple-500">{t.score.toFixed(1)}</span>
                      </div>
                    ))}
                  </div>
                </div>
      ...
      <header className="px-8 py-3 flex items-center justify-between bg-zinc-950 border-b border-white/5 flex-shrink-0 z-50">
        <div className="flex items-center gap-4">
          <img src="https://i.imgur.com/su8eRVi.png" alt="GTM" className="w-10 h-10 object-contain" />
          <div>
            <h1 className="text-base font-military font-bold text-purple-500 tracking-widest leading-none uppercase">GTM</h1>
            <p className="text-[7px] font-black text-slate-700 uppercase tracking-[0.3em] mt-1 italic">Grupamento Tático de Motocicletas</p>
          </div>
        </div>
        <nav className="flex bg-black p-1 rounded-xl border border-white/5">
          <button onClick={() => setActiveTab('evaluation')} className={`flex items-center gap-2 px-4 py-1.5 rounded-lg text-[8px] font-black uppercase tracking-widest transition-all ${activeTab === 'evaluation' ? 'bg-purple-500 text-black shadow-lg shadow-purple-500/20' : 'text-slate-600 hover:text-white'}`}>
            <FileText size={12}/> Avaliação
          </button>
          <button onClick={() => setActiveTab('protocols')} className={`flex items-center gap-2 px-4 py-1.5 rounded-lg text-[8px] font-black uppercase tracking-widest transition-all ${activeTab === 'protocols' ? 'bg-purple-500 text-black shadow-lg shadow-purple-500/20' : 'text-slate-600 hover:text-white'}`}>
            <HelpCircle size={12}/> Perguntas
          </button>
          <button onClick={() => setActiveTab('history')} className={`flex items-center gap-2 px-4 py-1.5 rounded-lg text-[8px] font-black uppercase tracking-widest transition-all ${activeTab === 'history' ? 'bg-purple-500 text-black shadow-lg shadow-purple-500/20' : 'text-slate-600 hover:text-white'}`}>
            <History size={12}/> Histórico
          </button>
        </nav>
        <button onClick={() => setShowSettings(true)} className="p-2 text-white/10 hover:text-purple-500/40 transition-all">
            ...
            <div className="col-span-12 lg:col-span-3 flex flex-col gap-3 overflow-y-auto custom-scrollbar pr-2">
              <section className="bg-zinc-900/40 border border-white/5 rounded-[1.2rem] p-4 space-y-2.5">
                <div className="flex justify-between items-center">
                  <h3 className="text-[10px] font-black text-purple-500 uppercase tracking-widest flex items-center gap-2"><Users size={12}/> Instrutores</h3>
                  <button onClick={() => setState(p => ({...p, instructors: [...p.instructors, {id:'', name:''}]}))} className="p-1 bg-purple-500/10 text-purple-500 rounded-md hover:bg-purple-500/20"><Plus size={10}/></button>
                </div>
                {state.instructors.map((ins, idx) => (
                  <div key={idx} className="space-y-1 group relative">
                    <input placeholder="NOME INSTRUTOR" className="w-full bg-black/60 border border-white/5 rounded-lg p-2 text-[10px] uppercase font-bold focus:border-purple-500 outline-none" value={ins.name} onChange={e => {
                      const n = [...state.instructors]; n[idx].name = e.target.value; setState({...state, instructors: n});
                    }} />
                    <input placeholder="ID" className="w-full bg-black/60 border border-white/5 rounded-lg p-2 text-[10px] font-bold focus:border-purple-500 outline-none" value={ins.id} onChange={e => {
              ...
              <section className="bg-zinc-900/40 border border-white/5 rounded-[1.2rem] p-4 space-y-2.5">
                <h3 className="text-[10px] font-black text-purple-500 uppercase tracking-widest flex items-center gap-2"><User size={12}/> Conscrito</h3>
                <input placeholder="NOME DO CONSRITO" className="w-full bg-black/60 border border-white/5 rounded-lg p-2 text-[10px] uppercase font-bold outline-none focus:border-purple-500" value={state.candidate.name} onChange={e => setState({...state, candidate: {...state.candidate, name: e.target.value}})} />
                <input placeholder="ID / PASSAPORTE" className="w-full bg-black/60 border border-white/5 rounded-lg p-2 text-[10px] font-bold outline-none focus:border-purple-500" value={state.candidate.id} onChange={e => setState({...state, candidate: {...state.candidate, id: e.target.value}})} />
              </section>
              ...
              <section className="bg-zinc-900/40 border border-white/5 rounded-[1.2rem] p-5 flex flex-col gap-3 mt-auto">
                <div className="flex justify-between items-start">
                  <div>
                    ...
                    <div className="text-4xl font-sans font-black text-purple-500 tracking-tighter">{finalScore.toFixed(1)}</div>
                  </div>
                  <div className="text-right">
                    <span className="text-[9px] font-black text-slate-700 uppercase block mb-1 tracking-widest">Média</span>
                    <input type="number" step="0.5" className="w-12 bg-black border border-white/5 rounded-lg p-1 text-center text-purple-500 text-[10px] font-bold outline-none" defaultValue="60" onChange={e => setPassingGrade(parseFloat(e.target.value) || 0)} />
                  </div>
                </div>
                ...
                <button onClick={handleFinish} disabled={isFinishing} className="w-full py-3 bg-purple-500 text-black rounded-xl font-black uppercase text-[9px] tracking-widest hover:bg-purple-400 disabled:opacity-50 transition-all flex items-center justify-center gap-2 shadow-lg shadow-purple-500/10">
            ...
            <div className="col-span-12 lg:col-span-4 flex flex-col gap-4 overflow-hidden">
              <section className="bg-zinc-900/30 border border-white/5 rounded-[1.5rem] p-5 space-y-3 flex-1 overflow-y-auto custom-scrollbar">
                <div className="flex justify-between items-center border-b border-white/5 pb-2">
                  <h3 className="text-[9px] font-black text-purple-500 uppercase tracking-widest flex items-center gap-2"><MapPin size={14}/> CIRCUITO DE RAMPAS</h3>
                  <button onClick={() => setState(p => ({...p, ramps: [...p.ramps, {uid:Math.random().toString(), name:'', score:0}]}))} className="p-1.5 bg-purple-500/10 text-purple-500 rounded-lg hover:bg-purple-500/20 transition-all"><Plus size={12}/></button>
                </div>
                <div className="space-y-1.5">
                  {state.ramps.map((ramp, idx) => (
                    <div key={ramp.uid} className="flex gap-3 items-center bg-black/60 border border-white/5 rounded-xl p-2.5 group hover:border-purple-500/20 transition-all">
                      ...
                      <div className="flex items-center gap-1.5 bg-zinc-950 border border-purple-500/40 rounded px-2.5 py-1 shadow-inner">
                        <span className="text-[8px] font-black text-purple-600 uppercase tracking-tighter">NOTA:</span>
                        <input type="number" step="0.5" className="w-10 bg-transparent text-center text-purple-400 text-sm font-black outline-none" value={ramp.score || ''} onChange={e => {
              ...
              <section className="bg-zinc-900/40 border border-white/5 rounded-[1.5rem] p-4 space-y-3">
                <h3 className="text-[9px] font-black text-purple-500 uppercase tracking-[0.2em] text-center border-b border-white/5 pb-2 flex items-center justify-center gap-2"><Target size={12}/> RESUMO OPERACIONAL</h3>
                <div className="space-y-1.5">
                  {[
                    ...
                    <div key={item.label} className="flex justify-between items-center bg-black/40 px-3 py-2 rounded-xl border border-white/5">
                      ...
                      <span className="text-sm font-black text-purple-500 tabular-nums">{item.score.toFixed(1)}</span>
                    </div>
                  ))}
                </div>
              </section>
            ...
            <div className="col-span-12 lg:col-span-5 flex flex-col gap-4 overflow-hidden">
              <div className="grid grid-cols-2 gap-3 flex-shrink-0">
                <div className="bg-zinc-900/30 border border-white/5 rounded-[1.5rem] p-4 space-y-2.5">
                  <h3 className="text-[9px] font-black text-purple-500 uppercase tracking-widest flex items-center gap-2"><Zap size={14}/> TUNEL</h3>
                  <div className="flex items-center gap-2 bg-black/60 border border-white/5 rounded-lg px-2.5 py-1.5 group/time">
                    <Clock className="text-purple-500/50 group-focus-within/time:text-purple-500 transition-colors shrink-0" size={14} />
                    <input placeholder="00:00" className="w-full bg-transparent text-[11px] font-black uppercase outline-none text-purple-500" value={state.tunnel.time} onChange={e => setState({...state, tunnel: {...state.tunnel, time: e.target.value}})} />
                  </div>
                  <div className="bg-zinc-950 border border-purple-500/40 rounded-lg p-1.5 flex items-center justify-between px-2.5">
                    <span className="text-[8px] font-black text-purple-600 uppercase tracking-tighter">NOTA:</span>
                    <input type="number" step="0.5" className="flex-1 bg-transparent text-sm font-black outline-none text-purple-400 text-center" value={state.tunnel.score || ''} onChange={e => setState({...state, tunnel: {...state.tunnel, score: parseFloat(e.target.value) || 0}})} />
                  </div>
                </div>
                <div className="bg-zinc-900/30 border border-white/5 rounded-[1.5rem] p-4 space-y-2.5">
                  <h3 className="text-[9px] font-black text-purple-500 uppercase tracking-widest flex items-center gap-2"><Activity size={14}/> MOD. CHINA</h3>
                  <div className="flex items-center gap-2 bg-black/60 border border-white/5 rounded-lg px-2.5 py-1.5 group/time">
                    <Clock className="text-purple-500/50 group-focus-within/time:text-purple-500 transition-colors shrink-0" size={14} />
                    <input placeholder="00:00" className="w-full bg-transparent text-[11px] font-black uppercase outline-none text-purple-500" value={state.modulation.time} onChange={e => setState({...state, modulation: {...state.modulation, time: e.target.value}})} />
                  </div>
                  <div className="bg-zinc-950 border border-purple-500/40 rounded-lg p-1.5 flex items-center justify-between px-2.5">
                    <span className="text-[8px] font-black text-purple-600 uppercase tracking-tighter">NOTA:</span>
                    <input type="number" step="0.5" className="flex-1 bg-transparent text-sm font-black outline-none text-purple-400 text-center" value={state.modulation.score || ''} onChange={e => setState({...state, modulation: {...state.modulation, score: parseFloat(e.target.value) || 0}})} />
                  </div>
                </div>
              </div>
              <section className="bg-zinc-900/30 border border-white/5 rounded-[1.5rem] p-5 space-y-4 flex-1 overflow-y-auto custom-scrollbar">
                <div className="flex justify-between items-center border-b border-white/5 pb-2">
                  <h3 className="text-[9px] font-black text-purple-500 uppercase tracking-widest flex items-center gap-2"><ClipboardCheck size={14}/> ACOMPANHAMENTOS</h3>
                  <button onClick={() => setState(p => ({...p, tracking: [...p.tracking, {uid:Math.random().toString(), location:'', time:'', score:0}]}))} className="p-1.5 bg-purple-500/10 text-purple-500 rounded-lg hover:bg-purple-500/20 transition-all"><Plus size={12}/></button>
                </div>
                <div className="space-y-2.5">
                  {state.tracking.map((track, idx) => (
                    <div key={track.uid} className="bg-black/60 border border-white/5 rounded-xl p-3 flex items-center justify-between group transition-all hover:border-purple-500/20">
                      <div className="flex-1 space-y-1.5">
                        ...
                        <div className="flex items-center gap-2 group/time">
                          <Clock className="text-purple-500/40 group-focus-within/time:text-purple-500 transition-colors" size={14} />
                          <input placeholder="00:00" className="bg-transparent outline-none text-[11px] text-purple-500 font-mono w-full" value={track.time} onChange={e => {
                      ...
                      <div className="flex items-center gap-3 border-l border-white/5 pl-3 ml-3">
                        <div className="flex flex-col items-center bg-zinc-950 border border-purple-500/30 rounded py-1.5 px-3 shadow-inner">
                          <span className="text-[7px] font-black text-purple-600 uppercase mb-0.5 tracking-tighter">NOTA</span>
                          <input type="number" step="0.5" className="w-10 bg-transparent text-center text-purple-400 text-lg font-black outline-none" value={track.score || ''} onChange={e => {
          ...
          <div className="max-w-4xl mx-auto h-full flex flex-col gap-4 animate-in fade-in slide-in-from-top-4 duration-500 overflow-hidden">
            <div className="bg-zinc-900/40 border border-white/5 rounded-[2rem] p-8 backdrop-blur-md overflow-hidden flex flex-col h-full shadow-2xl">
              <div className="flex items-center gap-4 mb-6 border-b border-white/5 pb-4">
                <div className="p-2.5 bg-purple-500/10 rounded-xl border border-purple-500/20"><HelpCircle className="text-purple-500" size={20}/></div>
              ...
              <div className="flex-1 overflow-y-auto custom-scrollbar pr-3 space-y-3">
                {protocols.map((p, idx) => (
                  <div key={idx} className={`border transition-all rounded-xl ${expandedProtocol === idx ? 'bg-purple-500/5 border-purple-500/30 shadow-[0_0_15px_rgba(245,158,11,0.05)]' : 'bg-black/60 border-white/5'}`}>
                    <button onClick={() => setExpandedProtocol(expandedProtocol === idx ? null : idx)} className="w-full text-left p-5 flex items-center justify-between gap-4">
                      <div className="flex-1">
                        <span className={`text-[10px] font-black px-2 py-0.5 rounded border mb-2 inline-block ${p.level === 'FÁCIL' ? 'text-green-500' : p.level === 'MÉDIO' ? 'text-purple-500' : 'text-red-500'}`}>{p.level}</span>
                        <h4 className={`text-base font-bold tracking-tight leading-snug ${expandedProtocol === idx ? 'text-purple-500' : 'text-slate-300'}`}>{p.q}</h4>
                      </div>
                      <ChevronDown className={`w-5 h-5 transition-transform shrink-0 ${expandedProtocol === idx ? 'rotate-180 text-purple-500' : ''}`} />
                    </button>
        ...
        {activeTab === 'history' && (
          <div className="max-w-5xl mx-auto h-full flex flex-col animate-in fade-in duration-500 overflow-hidden">
            <div className="flex items-center justify-between mb-6 border-b border-white/5 pb-4">
              <div className="flex items-center gap-4">
                <div className="p-2.5 bg-purple-500/10 rounded-xl border border-purple-500/20"><History className="text-purple-500" size={20}/></div>
                <div>
            ...
            <div className="flex-1 overflow-y-auto custom-scrollbar pr-2 space-y-2.5">
              {history.length === 0 ? (
                ...
                history.map(item => (
                  <div key={item.id} className="bg-zinc-900/40 border border-white/5 rounded-xl p-4 flex items-center justify-between group hover:border-purple-500/40 transition-all cursor-pointer hover:bg-zinc-900/60 shadow-lg shadow-black/20" onClick={() => { setSelectedResult(item); setShowResultModal(true); }}>
                    <div className="flex items-center gap-5">
                      ...
                      <div className="flex-1">
                        <div className="flex items-center gap-2">
                          <span className="text-[7px] font-black bg-purple-500/20 px-1.5 py-0.5 rounded text-purple-500 uppercase tracking-tighter">Conscrito</span>
                    ...
                    <div className="flex items-center gap-6">
                      ...
                      <div className="flex items-center gap-3">
                        <button type="button" className="p-2.5 text-slate-600 hover:text-purple-500 transition-colors bg-white/5 rounded-lg border border-white/5" title="Visualizar">
                          <Eye size={18}/>
                        </button>
      ...
      <footer className="px-8 py-1.5 border-t border-white/5 bg-black text-[6px] font-black text-slate-800 uppercase tracking-[0.4em] text-center flex-shrink-0">
        GTM - GRUPAMENTO TÁTICO DE MOTOCICLETAS - QUEBRADA
      </footer>
